...
return insertOk
    ...
    fun obtenerBitacorasPendientes(): List<BitacoraRecord> {
        ...
        db.rawQuery(queryPrincipal, null).use { cursor ->
            while (cursor.moveToNext()) {
                ...
                val idProgramada = cursor.getInt(cursor.getColumnIndexOrThrow("idRelProgramarActividadesBitacora"))
                ...
                db.rawQuery(queryUsuarios, arrayOf(idRegistro.toString())).use { userCursor ->
                ...
                db.rawQuery(queryFotos, arrayOf(idRegistro.toString())).use { photoCursor ->
    ...
    fun obtenerInspeccionUsuariosPendientes(): List<InspeccionUsuario> {
        val lista = mutableListOf<InspeccionUsuario>()
        val db = this.readableDatabase
        val cursor = db.rawQuery("SELECT * FROM inspeccion_usuarios WHERE sincronizado = 0", null)
        if (cursor.moveToFirst()) {
            do {
                val usuario = InspeccionUsuario(
                    id = cursor.getInt(cursor.getColumnIndexOrThrow("id")),
                    idUsuario = cursor.getInt(cursor.getColumnIndexOrThrow("idUsuario")),
                    fecha = cursor.getString(cursor.getColumnIndexOrThrow("fecha")),
                    // Leemos los nuevos campos desde la base de datos
    ...
    // Obtiene las actividades marcadas que están pendientes de sincronizar
    fun obtenerRelActividadesPendientes(): List<RelInspeccionActividad> {
        val lista = mutableListOf<RelInspeccionActividad>()
        val db = this.readableDatabase
        // La consulta no cambia, solo cómo leemos los datos
        val cursor = db.rawQuery("SELECT * FROM rel_inspeccion_actividades WHERE sincronizado = 0", null)

        if (cursor.moveToFirst()) {
            do {
                val actividad = RelInspeccionActividad(
                    // --- CAMPOS NUEVOS Y CORREGIDOS ---
                    id = cursor.getInt(cursor.getColumnIndexOrThrow("id")),
                    idInspeccionUsuarios = cursor.getInt(cursor.getColumnIndexOrThrow("idInspeccionUsuarios")),
                    idActividadInspeccion = cursor.getInt(cursor.getColumnIndexOrThrow("idActividadInspeccion")),
    ...
    // Marcar todo como sincronizado después de un envío exitoso
    fun marcarInspeccionesSincronizadas(idsUsuarios: List<Int>, idsActividades: List<Int>) {
        val db = this.writableDatabase
        val values = ContentValues().apply { put("sincronizado", 1) }

        // Creamos placeholders (?) para las consultas IN
        val userPlaceholders = idsUsuarios.joinToString(",") { "?" }
        val activityPlaceholders = idsActividades.joinToString(",") { "?" }

        // Convertimos las listas de Int a Array<String>
        val userArgs = idsUsuarios.map { it.toString() }.toTypedArray()
        val activityArgs = idsActividades.map { it.toString() }.toTypedArray()

        db.update("inspeccion_usuarios", values, "id IN ($userPlaceholders)", userArgs)
        db.update("rel_inspeccion_actividades", values, "id IN ($activityPlaceholders)", activityArgs)
        db.close()
    }


    fun marcarInspeccionesComoSincronizadas(usuarios: List<InspeccionUsuario>, actividades: List<RelInspeccionActividad>) {
        val db = this.writableDatabase
        db.beginTransaction() // Inicia la transacción
        try {
            val values = ContentValues().apply {
                put("sincronizado", 1) // El nuevo estado
            }

            // 1. Actualiza la tabla 'inspeccion_usuarios'
            for (usuario in usuarios) {
                db.update("inspeccion_usuarios", values, "id = ?", arrayOf(usuario.id.toString()))
            }

            // 2. Actualiza la tabla 'rel_inspeccion_actividades'
            for (actividad in actividades) {
                db.update("rel_inspeccion_actividades", values, "id = ?", arrayOf(actividad.id.toString()))
            }

            db.setTransactionSuccessful() // Confirma que todo salió bien
        } catch (e: Exception) {
            // Maneja cualquier error que pueda ocurrir
            Log.e("DBHelper", "Error al marcar inspecciones como sincronizadas", e)
        } finally {
